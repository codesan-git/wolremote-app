...
# Build for Linux
GOOS=linux GOARCH=amd64 go build -o $LINUX_OUTPUT cli/cmd/main.go

# Build for macOS (Apple Silicon processor)
GOOS=darwin GOARCH=arm64 go build -o $MAC_OUTPUT cli/cmd/main.go

# Build for Windows
GOOS=windows GOARCH=amd64 go build -o $WINDOWS_OUTPUT cli/cmd/main.go
